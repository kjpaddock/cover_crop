...
plot(fit.emm.trait)
fit.emm.threeway <- emmeans(aov.fit.mod.sqrt, pairwise ~ corn_trait|treatment|colony)
...
fig_leg2 <- get_legend(weight.plot.bar.sus)
ggsave(plot = fig_leg2, "/Users/PaddockK/Documents/Projects/Cover_crop/cover_crop_git/Figs/fig_legend.pdf", dpi = 300, width = 5, height = 3)

sum.weight.sus$order_value2 <- c(1,1.85,3.35,4.2)

weight.plot.bar.sus2 <-
  ggplot(aes(x = order_value2, y = mean_weight, fill = treatment, pattern = corn_trait), data = sum.weight.sus) +
  scale_fill_manual(values = c("#C3BD10", "#137098"), labels = c("Conservation", "Conventional")) +
  geom_bar_pattern(width=0.75, position = position_dodge(width = .9), stat = "identity",
                   color = "black", 
                   pattern_fill = "black",
...
ressus2 <- plot_grid(weight.plot.bar.sus2, weight.plot.bar.res, ncol = 1, align = "v", labels = c("a", "b"), label_size = 20)

testing2 <- ggdraw(ressus2) +
  geom_text( #Main effect stars for susceptible
    data = data.frame(x = 0.468, y = 0.95, label = "NS"),
    aes(x, y, label = label),
    size = 16/.pt,
    inherit.aes = FALSE
  ) +
   draw_line( # main effect line for susceptible
    x = c(0.325, 0.621),
    y = c(.94, 0.94),
    color = "black", size = 1
  ) +
  geom_text( #Main effect stars for resistant
    data = data.frame(x = 0.468, y = 0.455, label = "*"),
    aes(x, y, label = label),
    size = 20/.pt,
    inherit.aes = FALSE
  ) +
    draw_line( #main effect line for resistant
    x = c(0.325, 0.621),
    y = c(.45, 0.45),
    color = "black", size = 1
  ) +
   geom_text( #corn line effect stars for susceptible CC
    data = data.frame(x = 0.33, y = 0.905, label = "***"),
    aes(x, y, label = label),
    size = 20/.pt,
    inherit.aes = FALSE
  ) +
  draw_line( #corn line line for susceptible CC
    x = c(0.28, 0.38),
    y = c(.90, 0.90),
    color = "black", size = 1
  ) + 
   geom_text( #corn line effect stars for susceptible TR
    data = data.frame(x = 0.61, y = 0.86, label = "***"),
    aes(x, y, label = label),
    size = 20/.pt,
    inherit.aes = FALSE
  ) +
  draw_line( #corn line line for susceptible TR
    x = c(0.56, 0.66),
    y = c(.855, 0.855),
    color = "black", size = 1
  ) +
  geom_text( #corn line effect stars for resistant CC
    data = data.frame(x = 0.33, y = 0.385, label = "*"),
    aes(x, y, label = label),
    size = 20/.pt,
    inherit.aes = FALSE
  ) +
  draw_line( #corn line line for resistant CC
    x = c(0.28, 0.38),
    y = c(.38, 0.38),
    color = "black", size = 1
  ) +
   geom_text( #corn line effect stars for resistant TR
    data = data.frame(x = 0.61, y = 0.43, label = "NS"),
    aes(x, y, label = label),
    size = 16/.pt,
    inherit.aes = FALSE
  ) +
  draw_line( #corn line line for resistant TR
    x = c(0.56, 0.66),
    y = c(.42, 0.42),
    color = "black", size = 1
  )

save_plot(plot = testing2, "/Users/PaddockK/Documents/Projects/Cover_crop/cover_crop_git/Figs/230215_res_sus_weight_combined_DLF.pdf", base_width = 10, base_height = 15)
```
